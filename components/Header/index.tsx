...
const Header = ({ lng, data, docs }: Props) => {
  const menuData: Menu[] = [];
  ...
  return (
    <>
      ...
      <header
        className={`header left-0 z-40 flex w-full items-center bg-transparent   ${docs ? "justify-end !bg-[#585e6f] " : sticky
      ...
      >
        <div className="container">
          <div className="relative -mx-4 flex items-center justify-between">
            ...
            <div className={`flex w-full items-center  px-4 ${docs ? "justify-end":"justify-between"}`}>
              <div>
                <button
                      ...
                      <li key={menuItem.id} className="group relative">
                        {menuItem.path ? (
                          <Link
                            ...
                            className={`flex py-2 text-base text-dark group-hover:opacity-70 dark:text-white lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${docs ? "text-white": "dark:text-white text-dark"}`}
                          >
                            {menuItem.title}
                          </Link>
                        ) : (
                          <>
                            <a
                              ...
                              className={`flex cursor-pointer items-center justify-between py-2 text-base  group-hover:opacity-70  lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${docs ? "text-white": "dark:text-white text-dark"}`}
                            >
                              {menuItem.title}
                              <span className="pl-3">
                            ...
                            >
                              {menuItem.submenu?.map((submenuItem) => (
                                <Link
                                  ...
                                  key={submenuItem.id}
                                  className={`block rounded py-2.5 text-sm  hover:opacity-70  lg:px-3 ${docs ? "text-white": "text-dark dark:text-white"}`}
                                >
                                  {submenuItem.title}
                                </Link>
                              ))}
